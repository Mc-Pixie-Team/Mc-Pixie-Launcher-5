...
fn main() {
  ...
  .invoke_handler(tauri::generate_handler![my_custom_command, install_minecraft])
...
async fn my_custom_command(target: String){
 // let target = "https://upload.wikimedia.org/wikipedia/commons/thumb/1/11/Test-Logo.svg/783px-Test-Logo.svg.png";
  let file_name = "./downloaded_immg.jpg";
...
#[tauri::command]
async fn install_minecraft(version : String) {
  downloads::downloadlibraries().await;
